...
## awk 简介说明
- 功能
    + 逐行对文本和数据进行处理，每行称为一个记录。作用对象是流，可以是文件流，标准输入流或
      其他命令输出流。
- 基本结构
  ```shell
...
- 语句块
    ...
    + pattern 可以有两种写法 `{if(pattern){action}}` 或 `/pattern/{action}`。当要设置一些变量(
      进行标志或统计)以进行后续处理时，常使用前者；当只进行简单行扫描处理时，常使用后者。
...
- 其它运算符
  ...
  ```
- next 的使用
    + 循环逐行匹配，如果遇到next，就会跳出当前记录，执行下一条记录；
    + 可用于条件判断，也常用于行的合并；
- 流程控制语句
    + 包括条件判断语句，while循环语句，for循环语句，do...while循环语句几种主要语句，其用法
      和C语言相似
    + 不过for还有另外一种格式用法(无序输出)
      ```shell
        for(变量 in 数组)
          {变量在数组中为真}
      ```
    + 其他语句还包括 break，continue，next，exit等
- 数组应用及数组相关函数
    + awk数组不必提前声明，也不必声明大小;
    + 数组元素用0或空字符串来初始化，这根据上下文而定；
    + 特别注意，awk数组索引默认以1开始，这与C语言数组不同；
    + 可以使用数字或字符串作数组索引；
    + 遍历数组
      ```shell
