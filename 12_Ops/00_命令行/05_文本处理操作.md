...
- 常用命令
    ...
    + `find ./ -name "*.go" | xargs -I {} sed -e '' {} | wc -l`
        统计代码行数(不去除空行)
    + `echo '"front_enable":   false ,' | sed 's/\("front_enable":.*\)false\(.*\)/\1true\2/g'`
        将 false 替换为 true
    + `echo '"rot_x":   -0.0314 ,' | sed 's/\("rot_x":\s*\)[0-9\.\-]*\(\s*\)/\1-0.8932\2/g'`
        将 -0.0314 替换为 -0.8932
    + `sed -i "s/\(\"rot_x\":\s*\)[0-9\.\-]*\(\s*\)/\1${DOT_X}\2/g" tmp.json`
        通过变量传入进行替换
- 注意
