...
[参考: API](https://airflow.apache.org/docs/apache-airflow/2.1.3/security/api.html)
...
API 支持基于用户密码方式的 BA 认证。为了开启 BA 认证，需要修改一下配置。如下:
```cfg
    [api]
...
[Cross-origin resource sharing(CORS)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)是浏览器的一种安全机制，用于限制从浏览器中运行的脚本发起的 HTTP 请求。

`Access-Control-Allow-Headers`, `Access-Control-Allow-Methods`和`Access-Control-Allow-Origin` 头可以通过配置文件中的如下字段设置:
```cfg
    [api]
