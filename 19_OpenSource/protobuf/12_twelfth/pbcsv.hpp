...
bool csv2pb(T* t, const std::string& line,
            const std::set<std::string> & selectset = {},
            ...
            char kvsep = ':')
{
    ...
    for (const auto & kv : filtermap) {
        const google::protobuf::FieldDescriptor * f = t->GetDescriptor()->FindFieldByName(kv.first.c_str());
        if (! f) {
            LOGERRO("illegal %s", kv.first.c_str());
            return false;
        }
        if (_m.find(kv.first) == _m.end()) {
            continue;
        }
        switch (f->type()) {
            case google::protobuf::FieldDescriptor::Type::TYPE_DOUBLE:
            {
                double _cmp = std::strtod(kv.second.c_str(), nullptr);
                double _val = std::strtod(_m[kv.first].c_str(), nullptr);
                if (fabs(_cmp - _val) > CTRL_EPS) {
                    return false;
                }
            }
            break;
            case google::protobuf::FieldDescriptor::Type::TYPE_FLOAT:
            {
                float _cmp = std::strtof(kv.second.c_str(), nullptr);
                float _val = std::strtof(_m[kv.first].c_str(), nullptr);
                if (fabs(_cmp - _val) > CTRL_EPS) {
                    return false;
                }
            }
            ...
            case google::protobuf::FieldDescriptor::Type::TYPE_INT64:
            {
                int64_t _cmp = std::strtoll(kv.second.c_str(), nullptr, 0);
                int64_t _val = std::strtoll(_m[kv.first].c_str(), nullptr, 0);
                if (_cmp != _val) {
                    return false;
                }
            }
            ...
            case google::protobuf::FieldDescriptor::Type::TYPE_UINT64:
            {
                uint64_t _cmp = std::strtoull(kv.second.c_str(), nullptr, 0);
                uint64_t _val = std::strtoull(_m[kv.first].c_str(), nullptr, 0);
                if (_cmp != _val) {
                    return false;
                }
            }
            ...
            case google::protobuf::FieldDescriptor::Type::TYPE_INT32:
            {
                int32_t _cmp = std::atoi(kv.second.c_str());
                int32_t _val = std::atoi(_m[kv.first].c_str());
                if (_cmp != _val) {
                    return false;
                }
            }
            ...
            case google::protobuf::FieldDescriptor::Type::TYPE_UINT32:
            {
                uint32_t _cmp = static_cast<uint32_t>(std::atoi(kv.second.c_str()));
                uint32_t _val = static_cast<uint32_t>(std::atoi(_m[kv.first].c_str()));
                if (_cmp != _val) {
                    return false;
                }
            }
            ...
            case google::protobuf::FieldDescriptor::Type::TYPE_BOOL:
            {
                bool _cmp = std::atoi(kv.second.c_str());
                bool _val = std::atoi(_m[kv.first].c_str());
                if (_cmp != _val) {
                    return false;
                }
            }
    ...
    for (int i = 0; i < count; i++) {
        ...
        // 选择集过滤
        if (! selectset.empty() && selectset.find(f->name()) == selectset.end()) {
            continue;
        }
