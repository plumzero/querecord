...
TEST_F(RoutingActivationTest, ValidAddressParsing) {
	uint32_t expectedValue = 3858; // (0F12)16 = 3858)
	uint32_t address = 0;
...
TEST_F(RoutingActivationTest, WrongAddressParsing) {
	...
	ASSERT_EQ(result, _UnknownSourceAddressCode) <<"Wrong address doesn't return correct response code";
}
...
TEST_F(RoutingActivationTest, UnknownActivationType) {
	//Set wrong activation type(错误的激活类型)
	request[10] = 0xFF;
	unsigned char result = parseRoutingActivation(request + 8);	// 检测报文，要跳过通用报头
	ASSERT_EQ(result, _UnsupportedRoutingTypeCode);
}
...
TEST_F(RoutingActivationTest, ValidRequest) {
	unsigned char result = parseRoutingActivation(request + 8);	// 检测报文，要跳过通用报头
	ASSERT_EQ(result, _SuccessfullyRoutedCode);
}