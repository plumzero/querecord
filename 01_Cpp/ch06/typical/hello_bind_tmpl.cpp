...
// template<typename _Proto, typename _Ros, class _Func = std::function<void(const _Proto&, _Ros&)>>
template<typename _Proto, typename _Ros, class _Func = std::function<void(typename std::add_lvalue_reference<typename std::add_const<_Proto>::type>::type, typename std::add_lvalue_reference<_Ros>::type)>>
