...
using Params = std::geometric_distribution<>::param_type;
...
int main()
{
    ...
    while (true) {
        std::cout << "\nEnter values for the success, or Ctrl+Z to end(for example: 0.4):";
        if ((std::cin >> success).eof()) break;

        geometric.param(Params{ success });
        std::generate(std::begin(values), std::end(values), [&geometric, &rng]{ return geometric(rng); });
